...
export class RemoteControllerComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  characterFilter(){
    const characterList = this.getGameObjects(this.selectTab);
    // characterListの中からisStealthがtrueのもの以外をnotStealthCharacterに格納
    this.notStealthCharacter = characterList.filter( character => !character.isStealth );
  }
  ...
  trackByNotStealthCharacter(index: number, gameObject: GameObject) {
    return gameObject ? gameObject.identifier : index;
  }
