...
# UdonariumNオリジナル要素
■特定のゲームシステムのキャラクター駒生成
└現在は獸ノ森、インセイン、スカイノーツに対応
　駒にマウスオーバーした際に出るポップアップに表示される項目も自動で設定します。
■特定のゲームシステムに合わせた仕様
・獸ノ森
├チャットパレットに継続判定ボタンを表示
　直前の判定の出目を検出し、ボタンをクリックするだけで軽率に継続判定ができます。　ギャンブル最高！
├先制値決定の判定を行った際、自動で駒のステータスに代入
　インベントリの表示を先制値順ソートにすることで行動順がひと目でわかるように

・インセイン
└特技表（製作途中）
　特技表を表示してそこから判定を行えるように

■ダブルクリックアクション
・キャラクター駒をダブルクリックした際に行うアクションを設定可能に
　詳細を表示、チャットパレットを表示、ステルスモード切り替えなど設定することが可能。

■ダイス判定のトースト通知
・ダイス判定を行った際、結果を画面右下に通知表示　成功は緑色、失敗は赤色で表示され一目でわかるように。
　トースト通知が邪魔な場合は個人設定でオフにできます。

■画面右上にBGM音量調節バー

他、高度機能や射程、バフ・デバフ、タイマー機能など
# ユドナリウム
